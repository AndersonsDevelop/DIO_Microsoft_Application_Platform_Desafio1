...
def insert_product(product_name, product_pŕice, product_description, product_image):
    try:
        conn = pymssql.connect(server=SQL_SERVER, user=SQL_USER, password=SQL_PASSWORD, database=SQL_DATABASE)
        cursor = conn.cursor()
        query = "INSERT INTO dbo.Produtos (nome, preco, descricao, image_url) VALUES (%s, %s, %s, %s)"
        cursor.execute(query, (product_name, product_price, product_description, product_image))
        conn.commit()
        conn.close()
...
if st.button('Salvar Produto'):
    if product_image is not None:
        product_image_url = upload_blob(product_image)
    else:
        product_image_url = None

    success = insert_product(product_name, product_price, product_description, product_image_url)
    if success:
